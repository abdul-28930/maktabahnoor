...
export async function GET(req) {
  ...
  try {
    ...
    return NextResponse.json({ clothing: list });
  } catch { return NextResponse.json({ clothing: [] }); }
}
  ...
  try {
    ...
    const item = {
      ...
      variants: Array.isArray(data.variants) ? data.variants
        ...
        .map(v => ({ id: v.id || uid(), label: v.label.trim(), size: v.size?.trim() || '', color: v.color || '', stockCount: Math.max(0, parseInt(v.stockCount) || 0) }))
        : [],
