...
// Don't store base64 images in meta — keeps meta index small
...
export async function GET(req) {
  ...
  try {
    ...
    let list = all ? meta : meta.filter(b => b.inStock !== false);
...
export async function POST(req) {
  try {
    ...
    const book = {
      ...
      tags:        data.tags || [],
      coverUrl:    data.coverUrl || '',   // full base64 stored only here
      createdAt:   now,
      updatedAt:   now,
    };

    // Store full book (including base64 cover) in its own key
    await redis.set(`mn_book:${slug}`, book);

    // Meta index stores only lightweight fields — NO base64
    const meta = await redis.get('mn_books_meta') || [];
    ...
    const m    = {
      ...
      tags:     book.tags,
      coverUrl: safeUrl(book.coverUrl), // '__base64__' or actual URL — never raw base64
      createdAt: now,
    };
    ...
    console.error('POST /api/books error:', e?.message, e?.stack);
