...
export async function POST(req) {
  try {
    ...
    function replaceTranslator(current = '', target, replacement) {
    ...
    for (const b of affected) {
      ...
      if (full) {
        const nextVal = replaceTranslator(full[FIELD], oldName, clean);
    ...
    await redis.set('mn_books_meta', meta.map(b => {
      ...
      if (parts.includes(oldName.toLowerCase()) || b[FIELD] === oldName) {
        return { ...b, [FIELD]: replaceTranslator(b[FIELD], oldName, clean) };
      }
      return b;
    }));
