    ...
    <div style={{position:'relative',minHeight:'100vh',background:'#faf9f5',fontFamily:"'DM Sans',sans-serif",overflowX:'hidden'}}>
      ...
      <nav style={{position:'sticky',top:0,zIndex:40,height:68,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 clamp(20px,5vw,72px)',backdropFilter:'blur(10px)',background:'rgba(250,249,245,0.85)',borderBottom:'1px solid rgba(27,67,50,0.08)'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:24}}>
          <Link href="/"      style={{textDecoration:'none',fontSize:13,color:'#6b6460',letterSpacing:.3}} onMouseEnter={e=>e.target.style.color='#b8965a'} onMouseLeave={e=>e.target.style.color='#6b6460'}>Home</Link>
          <Link href="/books" style={{textDecoration:'none',fontSize:13,color:'#6b6460',letterSpacing:.3}} onMouseEnter={e=>e.target.style.color='#b8965a'} onMouseLeave={e=>e.target.style.color='#6b6460'}>Collection</Link>
        </div>
      </nav>
      ...
      <div style={{position:'relative',zIndex:1,padding:'20px clamp(20px,5vw,72px)',borderBottom:'1px solid rgba(27,67,50,0.06)'}}>
        <div style={{maxWidth:1200,margin:'0 auto',display:'flex',alignItems:'center',gap:10,fontSize:12,color:'#a09890',letterSpacing:.5}}>
          <Link href="/" style={{textDecoration:'none',color:'#a09890',transition:'color .2s'}} onMouseEnter={e=>e.target.style.color='#b8965a'} onMouseLeave={e=>e.target.style.color='#a09890'}>Home</Link>
          <span style={{color:'rgba(27,67,50,0.2)'}}>›</span>
          <Link href="/books" style={{textDecoration:'none',color:'#a09890',transition:'color .2s'}} onMouseEnter={e=>e.target.style.color='#b8965a'} onMouseLeave={e=>e.target.style.color='#a09890'}>Collection</Link>
          <span style={{color:'rgba(27,67,50,0.2)'}}>›</span>
      ...
      {/* MAIN */}
      <div style={{position:'relative',zIndex:1,maxWidth:1200,margin:'0 auto',padding:'56px clamp(20px,5vw,72px) 100px',display:'grid',gridTemplateColumns:'320px 1fr',gap:72,alignItems:'start'}}>
        ...
        <div style={{position:'sticky',top:92}}>
          <div style={{borderRadius:18,overflow:'hidden',boxShadow:'0 20px 60px rgba(27,67,50,0.2)',animation:'floatBook 6s ease-in-out infinite',aspectRatio:'3/4'}}>
            {book.coverUrl ? (
              ...
              <div style={{width:'100%',height:'100%',background:COVER_BG,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:16,padding:32}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:8,marginTop:20,justifyContent:'center'}}>
              {book.tags.map(t => (
                <span key={t} style={{padding:'5px 14px',borderRadius:20,fontSize:10,fontWeight:500,letterSpacing:1,textTransform:'uppercase',
                  background:t==='Bestseller'?'rgba(184,150,90,0.12)':t==='New Arrival'?'rgba(27,67,50,0.08)':'rgba(184,150,90,0.08)',
                  color:t==='Bestseller'?'#b8965a':t==='New Arrival'?'#1b4332':'#b8965a',
        ...
        <div>
          ...
          {/* Title */}
          <h1 style={{margin:'0 0 8px',fontFamily:"'Cormorant Garamond',serif",fontWeight:500,fontSize:'clamp(32px,4.5vw,52px)',color:'#1b4332',lineHeight:1.12,letterSpacing:.02}}>
            {book.title}
          </h1>
          {book.titleAr && (
          ...
          <div style={{display:'flex',gap:14,flexWrap:'wrap'}}>
            <a href={`${IG_URL}?text=Assalamualaikum, I would like to order: ${encodeURIComponent(book.title)}`} target="_blank" rel="noreferrer"
              style={{textDecoration:'none',display:'inline-flex',alignItems:'center',gap:10,background:'#1b4332',color:'#fff',padding:'16px 32px',borderRadius:40,fontSize:14,letterSpacing:.4,fontWeight:400,transition:'background .2s,transform .15s,box-shadow .2s',boxShadow:'0 6px 20px rgba(27,67,50,0.25)'}}
            ...
            <Link href="/books"
              style={{textDecoration:'none',display:'inline-flex',alignItems:'center',gap:8,border:'1.5px solid rgba(27,67,50,0.2)',color:'#1b4332',padding:'16px 28px',borderRadius:40,fontSize:14,letterSpacing:.4,transition:'all .2s'}}
              onMouseEnter={e=>{e.currentTarget.style.background='rgba(27,67,50,0.05)';e.currentTarget.style.borderColor='#1b4332';}}
              onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.borderColor='rgba(27,67,50,0.2)';}}>
              ← Back to Collection
            </Link>
          </div>

          {/* Instagram note */}
          <div style={{marginTop:24,padding:'16px 20px',background:'rgba(184,150,90,0.06)',borderRadius:12,border:'1px solid rgba(184,150,90,0.18)',display:'flex',alignItems:'flex-start',gap:12}}>
