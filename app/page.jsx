...
const DISPLAY_CATS = [
  {name:'Tafsīr',ar:'تفسير'},{name:'Hadith',ar:'حديث'},
  {name:'Seerah',ar:'سيرة'},{name:'Fiqh',ar:'فقه'},
  {name:'Aqeedah',ar:'عقيدة'},{name:'Duʿā & Dhikr',ar:'دعاء'},
  {name:'Manners',ar:'أخلاق'},{name:'Arabic Language',ar:'لغة'},
];
const NAV = ['Collection','Categories','New Arrivals','About'];
const MARQUEE_ITEMS = [
  'Arabic Books','Urdu Books','Hadith Collections','Tafsīr','ʿAqīdah',
  'Seerah','Fiqh','Manners & Character','New Arrivals',
  'Arabic Books','Urdu Books','Hadith Collections','Tafsīr','ʿAqīdah',
  'Seerah','Fiqh','Manners & Character','New Arrivals',
];
...
function BookCard({ book, idx }) {
  ...
  return (
    <Link href={book.slug !== '#' ? `/book/${book.slug}` : '#'} className="hp-book-card hp-reveal" style={{textDecoration:'none',color:'inherit',background:'#fff',borderRadius:14,overflow:'hidden',border:'1px solid rgba(27,67,50,0.07)',boxShadow:'0 10px 24px rgba(27,67,50,0.06)',cursor:'pointer',display:'block'}} data-reveal data-reveal-delay={idx * 0.1}>
...
export default function HomePage() {
  ...
  // Fetch data
  useEffect(() => {
    fetch('/api/books?tag=Featured')
      .then(r => r.json()).then(d => setFeaturedBooks(d.books?.slice(0,4) || [])).catch(()=>{});
    fetch('/api/books?tag=New+Arrival')
      .then(r => r.json()).then(d => setNewArrivals(d.books?.slice(0,4) || [])).catch(()=>{});
  }, []);

  // Scroll reveals + counter + parallax
  useEffect(() => {
  ...
  return (
    <div ref={rootRef} style={{position:'relative',minHeight:'100vh',background:'#faf9f5',color:'#1a1712',fontFamily:"'DM Sans',sans-serif",overflowX:'hidden',animation:'pageFade .5s ease both'}}>
      ...
      <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:40,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'20px clamp(20px,5vw,72px)',backdropFilter:'blur(8px)',background:'rgba(250,249,245,0.72)',borderBottom:'1px solid rgba(27,67,50,0.07)'}}>
        <Link href="/" className="hp-nlink" style={{display:'flex',alignItems:'center',gap:12,textDecoration:'none',color:'#1b4332'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:16}}>
          <Link href="/books" className="hp-nlink hp-glow-cta" style={{display:'none'}}></Link>
          <button className="hp-hamburger" onClick={()=>setMenuOpen(o=>!o)} aria-label="Menu" style={{display:'none',flexDirection:'column',gap:5,width:42,height:42,alignItems:'center',justifyContent:'center',background:'transparent',border:'1px solid rgba(27,67,50,0.18)',borderRadius:10,cursor:'pointer'}}>
            <span style={{width:18,height:'1.6px',background:'#1b4332',display:'block'}}/>
            <span style={{width:18,height:'1.6px',background:'#1b4332',display:'block'}}/>
            <span style={{width:18,height:'1.6px',background:'#1b4332',display:'block'}}/>
          </button>
        </div>
      ...
      <header id="top" style={{position:'relative',zIndex:1,minHeight:'100vh',display:'flex',alignItems:'center',padding:'140px clamp(20px,5vw,72px) 130px'}}>
        ...
        <div style={{position:'absolute',inset:0,overflow:'hidden',pointerEvents:'none'}} data-calli="true">
      ...
      <section id="categories" style={{position:'relative',zIndex:1,background:'#1b4332',padding:'96px clamp(20px,5vw,72px)',overflow:'hidden'}}>
        ...
        <div className="hp-four-grid" style={{position:'relative',maxWidth:1180,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:22}}>
          {DISPLAY_CATS.map((cat,i)=>(
            <Link key={cat.name} href={`/books?category=${encodeURIComponent(cat.name)}`} className="hp-cat-card hp-reveal" style={{opacity:0,transform:'translateY(24px)',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(184,150,90,0.32)',borderRadius:14,padding:'32px 20px',textAlign:'center',cursor:'pointer',textDecoration:'none',display:'block'}} data-reveal data-reveal-delay={i*0.07}>
      ...
      <section id="new-arrivals" style={{position:'relative',zIndex:1,padding:'96px clamp(20px,5vw,72px)',background:'#f4f1e9',overflow:'hidden'}}>
        <svg width="420" height="420" viewBox="0 0 200 200" style={{position:'absolute',top:-40,right:-40,opacity:.5,pointerEvents:'none'}} aria-hidden="true">
          <path d="M40 200 L40 90 A60 60 0 0 1 160 90 L160 200" fill="none" stroke="rgba(27,67,50,0.05)" strokeWidth="2"/>
        </svg>
        <div className="hp-reveal" style={{position:'relative',textAlign:'center',marginBottom:54}}>
        ...
        <div style={{position:'relative',background:'#f4f1e9',padding:'60px clamp(28px,5vw,60px)',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
          <div data-parallax="0.12" style={{width:'100%',maxWidth:420}}>
            <svg width="100%" viewBox="0 0 420 320" aria-hidden="true">
              <g stroke="#1b4332" strokeWidth="1.4" fill="none" opacity="0.8">
                <rect x="40" y="40" width="340" height="240" rx="4"/>
                <line x1="40" y1="120" x2="380" y2="120"/><line x1="40" y1="200" x2="380" y2="200"/>
              </g>
              <g>
                <rect x="58"  y="56"  width="20" height="56" rx="2" fill="#1b4332"/>
                <rect x="82"  y="50"  width="18" height="62" rx="2" fill="#2d6a4f"/>
                <rect x="104" y="58"  width="22" height="54" rx="2" fill="#b8965a"/>
                <rect x="130" y="52"  width="16" height="60" rx="2" fill="#1b4332"/>
                <rect x="150" y="56"  width="20" height="56" rx="2" fill="#d4ab70"/>
                <rect x="174" y="50"  width="18" height="62" rx="2" fill="#2d6a4f"/>
                <rect x="58"  y="136" width="18" height="56" rx="2" fill="#b8965a"/>
                <rect x="80"  y="130" width="20" height="62" rx="2" fill="#1b4332"/>
                <rect x="104" y="138" width="16" height="54" rx="2" fill="#2d6a4f"/>
                <rect x="124" y="132" width="22" height="60" rx="2" fill="#d4ab70"/>
                <rect x="300" y="56"  width="20" height="56" rx="2" fill="#2d6a4f"/>
                <rect x="324" y="50"  width="18" height="62" rx="2" fill="#b8965a"/>
                <rect x="346" y="58"  width="20" height="54" rx="2" fill="#1b4332"/>
                <rect x="300" y="136" width="18" height="56" rx="2" fill="#d4ab70"/>
                <rect x="322" y="130" width="20" height="62" rx="2" fill="#1b4332"/>
                <rect x="346" y="138" width="20" height="54" rx="2" fill="#2d6a4f"/>
              </g>
              <g>
      ...
      <section style={{position:'relative',zIndex:1,background:'#15291f',padding:'90px clamp(20px,5vw,72px)',textAlign:'center',overflow:'hidden'}}>
        <div className="hp-reveal" style={{position:'relative',maxWidth:880,margin:'0 auto'}}>
          ...
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontStyle:'italic',fontSize:'clamp(34px,5vw,52px)',color:'#b8965a',marginBottom:36}}>{IG_HANDLE}</div>
      ...
      {/* ── MOBILE MENU ── */}
      <div ref={backdropRef} onClick={()=>setMenuOpen(false)} style={{position:'fixed',inset:0,zIndex:50,background:'rgba(21,41,31,0.55)',backdropFilter:'blur(3px)',opacity:0,pointerEvents:'none',transition:'opacity .35s ease'}}/>
      <div ref={menuRef} style={{position:'fixed',top:0,right:0,bottom:0,zIndex:51,width:'min(82vw,340px)',background:'#faf9f5',boxShadow:'-20px 0 50px rgba(27,67,50,0.2)',transform:'translateX(100%)',transition:'transform .4s cubic-bezier(.4,0,.2,1)',padding:'30px 28px',display:'flex',flexDirection:'column'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:40}}>
          <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:600,color:'#1b4332'}}>Menu</span>
          <button onClick={()=>setMenuOpen(false)} aria-label="Close" style={{width:40,height:40,border:'1px solid rgba(27,67,50,0.18)',borderRadius:10,background:'transparent',fontSize:20,color:'#1b4332',cursor:'pointer'}}>✕</button>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:6}}>
          {NAV.map(n=>(
            <a key={n} href={`#${n.toLowerCase().replace(/ /g,'-')}`} onClick={()=>setMenuOpen(false)} className="hp-nlink" style={{textDecoration:'none',fontFamily:"'Cormorant Garamond',serif",fontSize:26,color:'#1b4332',padding:'10px 0',borderBottom:'1px solid rgba(27,67,50,0.08)'}}>{n}</a>
          ))}
          <Link href="/books" onClick={()=>setMenuOpen(false)} style={{textDecoration:'none',fontFamily:"'Cormorant Garamond',serif",fontSize:26,color:'#1b4332',padding:'10px 0',borderBottom:'1px solid rgba(27,67,50,0.08)'}}>All Books</Link>
        </div>
        <div dir="rtl" style={{marginTop:'auto',fontFamily:"'Noto Naskh Arabic',serif",fontSize:26,color:'#b8965a',textAlign:'center'}}>مكتبة النور</div>
      </div>
