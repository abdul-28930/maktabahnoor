...
export function splitTranslators(str = '') {
  return String(str || '').split(',').map(s => s.trim()).filter(Boolean);
}
...
// Helper to normalize book categories whether saved as array `categories` or legacy single `category` string.
export function getBookCategories(book) {
  if (!book) return [];
  ...
  if (typeof book.category === 'string' && book.category.trim()) {
    return book.category.split(',').map(c => c.trim()).filter(Boolean);
  }
  if (Array.isArray(book.category) && book.category.length > 0) {
    return book.category.filter(Boolean);
  }
  return [];
}
...
export const META_COVER_MAX_CHARS = 400000; // ~300KB base64
